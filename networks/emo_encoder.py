...
from networks.basic_modules import get_norm, get_act, ConvNormAct, LayerScale2D, MSPatchEmb
...
class EMO(nn.Module):
    ...
    def __init__(self, dim_in=3,
                 ...
                 attn_drop=0., drop=0., drop_path=0., v_group=False, attn_pre=False, **kwargs):
        super().__init__()
...
def emo_xx_small(pretrained_weights="", **kwargs):
    # pretrain weight link   实例化不同规模模型
    # https://github.com/zhangzjn/EMO/raw/main/resources/EMO_1M/net.pth
    model = EMO(
                # dim_in=3
                ...
                **kwargs) # attn_pre=True
    pretrained_weights_path = os.path.join(pretrained_weights, "EMO_1M.pth")
    ...
    model.num_ch_enc = [24, 32, 48, 80, 168]  # xxs channel
    return model


def emo_x_small(pretrained_weights="", **kwargs):
    # pretrain weight link
    # https://github.com/zhangzjn/EMO/raw/main/resources/EMO_2M/net.pth
    model = EMO(
                # dim_in=3,
                ...
                qkv_bias=True, attn_drop=0., drop=0., drop_path=0.05, v_group=False, attn_pre=False,
                **kwargs) # attn_pre=True
    pretrained_weights_path = os.path.join(pretrained_weights, "EMO_2M.pth")
    if pretrained_weights != "":
        assert os.path.exists(pretrained_weights_path), "weights file not exist."
        weights_dict = torch.load(pretrained_weights_path, map_location="cuda:0")
        weights_dict = weights_dict["model"] if "model" in weights_dict else weights_dict
    ...
    model.num_ch_enc = [24, 32, 48, 120, 200]  # xs channel
    return model


def emo_small(pretrained_weights="", **kwargs):
    # pretrain weight link
    # https://github.com/zhangzjn/EMO/raw/main/resources/EMO_5M/net.pth
    model = EMO(
                # dim_in=3,
                ...
                qkv_bias=True, attn_drop=0., drop=0., drop_path=0.05, v_group=False, attn_pre=False,
                **kwargs)  # attn_pre=True
    pretrained_weights_path = os.path.join(pretrained_weights, "EMO_5M.pth")
    if pretrained_weights != "":
        assert os.path.exists(pretrained_weights_path), "weights file not exist."
        weights_dict = torch.load(pretrained_weights_path, map_location="cuda:0")
        weights_dict = weights_dict["model"] if "model" in weights_dict else weights_dict
...
# def emo_small(pretrained_weights="", **kwargs):
#     # pretrain weight link
#     # https://github.com/zhangzjn/EMO/raw/main/resources/EMO_6M/net.pth
#     model = EMO(
#                 # dim_in=3,
...
#                 qkv_bias=True, attn_drop=0., drop=0., drop_path=0.05, v_group=False, attn_pre=False,
#                 **kwargs) # attn_pre=True
#     pretrained_weights_path = os.path.join(pretrained_weights, "EMO_6M.pth")
#     if pretrained_weights != "":
#         assert os.path.exists(pretrained_weights_path), "weights file not exist."
#         weights_dict = torch.load(pretrained_weights_path, map_location="cuda:0")
#         weights_dict = weights_dict["model"] if "model" in weights_dict else weights_dict
...
#                 del weights_dict[k]
#         model.load_state_dict(weights_dict, strict=False)
#     model.num_ch_enc = [24, 48, 72, 160, 320]  # s channel
#
#     return model
