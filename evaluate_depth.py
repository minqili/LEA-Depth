...
def evaluate(opt):
    ...
    if opt.ext_disp_to_eval is None:
        ...
        if opt.encoder_emo == "s":
            encoder = networks.emo_small(opt.weights_dir)
        elif opt.encoder_emo == "xs":
            encoder = networks.emo_x_small(opt.weights_dir)
        elif opt.encoder_emo == "xxs":
            encoder = networks.emo_xx_small(opt.weights_dir)
        ...
        # ====== Speed config ======
        warmup_batches = 5       # 前5个batch热身
        measure_batches = 50     # 统计接下来50个batch（想全测就改成 len(dataloader)）
        enc_times_ms = []
        dec_times_ms = []
        full_times_ms = []
        ...
        # 只 profile 一次（不要每个 batch 都 profile）
        flops = params = flops_e = params_e = flops_d = params_d = None
        profiled = False

        with torch.no_grad():
            for batch_idx, data in enumerate(dataloader):
                input_color = data[("color", 0, 0)].cuda(0, non_blocking=True)
                ...
                # profile 只做一次
                if not profiled:
                    flops, params, flops_e, params_e, flops_d, params_d = profile_once(
                        encoder, depth_decoder, input_color
                    )
                    profiled = True

                # 是否统计时间
                do_measure = (batch_idx >= warmup_batches) and (batch_idx < warmup_batches + measure_batches)
                ...
                if do_measure:
                    ...
                    # ms per batch -> ms per image
                    bs = input_color.shape[0]
                    enc_times_ms.append(enc_start.elapsed_time(enc_end) / bs)
                    dec_times_ms.append(dec_start.elapsed_time(dec_end) / bs)
                    full_times_ms.append(full_start.elapsed_time(full_end) / bs)
        ...
        # ====== Print speed results ======
        def report_speed(name, times_ms):
            if len(times_ms) == 0:
                print(f"{name}: no timing samples collected.")
                return
            avg = float(np.mean(times_ms))
            std = float(np.std(times_ms))
            fps = 1000.0 / avg
            print(f"{name}: {avg:.3f} ms/img (std {std:.3f}), FPS: {fps:.2f}")

        print("\n====== Inference Speed (forward only, per-image) ======")
        report_speed("Encoder", enc_times_ms)
        report_speed("Decoder", dec_times_ms)
        report_speed("Full (Enc+Dec)", full_times_ms)
        print("======================================================\n")
